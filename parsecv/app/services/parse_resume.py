from app.models import Education, Experience, Skills, Applicant, Resume
...
def parse_resume(file: BinaryIO):
    ...
    if extractor_response.status_code != 200:
        ...
        raise ParsingException(status=500, detail="There was an error calling the backend text extractor service")
    ...
    applicant_labels = ["CONTACT_EMAIL", "CONTACT_PHONE", "GITHUB", "LINKEDIN", "WEBSITE"]
    ...
    resume = Resume(info=applicant, overview=overview, skills=skills, experience=experience_items, education=education_items)
